...
// https://github.com/joel-bitar/react-native-apple-ads-attribution
            ...
            if (statusCode != 200) {
                if ((statusCode == 404 || statusCode == 500) && retriesLeft > 0) {
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        [ExpoAppleSearchAdsAttribution requestAdServicesAttributionDataUsingToken:token retriesLeft:retriesLeft-1 completionHandler:completionHandler];
                    });
                } else {
...
+ (void) getAdServicesAttributionDataWithCompletionHandler: (void (^)(NSDictionary * _Nullable data, NSError * _Nullable error))completionHandler {
    ...
    if (@available(iOS 14.3, *))
    {
        ...
        NSString* attributionToken = [ExpoAppleSearchAdsAttribution getAdServicesAttributionToken:&tokenError];
        
        if (attributionToken) {
            [ExpoAppleSearchAdsAttribution requestAdServicesAttributionDataUsingToken:attributionToken retriesLeft:NUM_RETRIES completionHandler:completionHandler];
        } else {
            // No token
...
RCT_EXPORT_METHOD(getAttributionData:
    ...
    [ExpoAppleSearchAdsAttribution getAdServicesAttributionDataWithCompletionHandler:^(NSDictionary * _Nullable attributionData, NSError * _Nullable adServicesError) {
        if (attributionData != nil) {
            resolve(attributionData);
        } else {
            // Fallback to old iAd client API
            [ExpoAppleSearchAdsAttribution getiAdAttributionDataWithCompletionHandler:^(NSDictionary * _Nullable data, NSError * _Nullable iAdError) {
                if (data != nil) {
                    resolve(data);
                } else {
                    ...
                    [ExpoAppleSearchAdsAttribution rejectPromiseWithUserInfo:reject
                                                          userInfo:[@{
                                                            @"code" : @"unknown",
...
RCT_EXPORT_METHOD(getiAdAttributionData: (RCTPromiseResolveBlock) resolve rejecter: (RCTPromiseRejectBlock) reject) {
    ...
    [ExpoAppleSearchAdsAttribution getiAdAttributionDataWithCompletionHandler:^(NSDictionary * _Nullable data, NSError * _Nullable error) {
        if(data != nil) {
            resolve(data);
        } else {
            [ExpoAppleSearchAdsAttribution rejectPromiseWithNSError:reject error:error];
        }
...
RCT_EXPORT_METHOD(getAdServicesAttributionToken: (RCTPromiseResolveBlock) resolve rejecter: (RCTPromiseRejectBlock) reject) {
    NSError *error = nil;
    NSString* attributionToken = [ExpoAppleSearchAdsAttribution getAdServicesAttributionToken:&error];
...
RCT_EXPORT_METHOD(getAdServicesAttributionData: (RCTPromiseResolveBlock) resolve rejecter: (RCTPromiseRejectBlock) reject) {
    [ExpoAppleSearchAdsAttribution getAdServicesAttributionDataWithCompletionHandler:^(NSDictionary * _Nullable attributionData, NSError * _Nullable error) {
        if (attributionData != nil) {
            resolve(attributionData);
        } else {
            [ExpoAppleSearchAdsAttribution rejectPromiseWithNSError:reject error:error];
